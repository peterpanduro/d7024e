...
import (
    ...
    "encoding/hex"
    "time"
    "d7024e/helpers"  // Importing helpers for error handling
)
...
func (kademlia *Kademlia) LookupData(hash string) *MsgData {
    ...
    for _, contact := range closestContacts {
        go func(contact *Contact) {
            ...
            // Placeholder logic simulating data reception from contact
...
func (kademlia *Kademlia) Store(data []byte) string {
    ...
    for _, contact := range closestContacts {
        go func(contact *Contact) {
            ...
            err := SendMessage(message)
            if err != nil {
                // Handle network errors, but ignore in this case
                return
            }
        }(contact)
    }
